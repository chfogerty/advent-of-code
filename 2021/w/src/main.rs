// stolen from https://github.com/Mesoptier/advent-of-code-2021/blob/master/src/days/day23.rs
...
fn parse_data(input: &str) -> Vec<Amphipod> {
    let amphipods = input
        .chars()
...
//fn part_a(input: &Vec<Amphipod>) -> usize {
fn part_a(input: &str) -> usize {
    let amphipods = parse_data(input);
    ...
    let (score, moves) = solve_both_parts(initial_state);
    print_moves::<2>(&moves);
    score
}
...
fn part_b(input: &str) -> usize {
    ...
    let initial_state = State {
        hallway: [None; 11],
        rooms: [
            [amphipods[0], Amphipod::D, Amphipod::D, amphipods[4]].map(|a| Some(a)),
            [amphipods[1], Amphipod::C, Amphipod::B, amphipods[5]].map(|a| Some(a)),
            [amphipods[2], Amphipod::B, Amphipod::A, amphipods[6]].map(|a| Some(a)),
            [amphipods[3], Amphipod::A, Amphipod::C, amphipods[7]].map(|a| Some(a)),
        ],
    };
...
fn print_moves<const R: usize>(moves: &Vec<u64>) {
    for mv in moves {
        let state = State::<R>::decode(*mv);
        println!("{}\n", state);
    }
}
...
fn solve_both_parts<const R: usize>(initial_state: State<R>) -> (usize, Vec<u64>) {
    ...
    }) = q.pop()
    {
        if encoded_state == encoded_goal_state {
            ...
            new_moves.push(encoded_state);
            return (f_score, new_moves);
        }
